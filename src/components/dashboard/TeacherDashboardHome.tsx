...
export default function TeacherDashboardHome() {
  ...
  const displayName = data?.user?.name ?? data?.user?.email ?? "Docente";
  const aula =
    data?.user?.assignedGradeName && data?.user?.assignedSectionName
      ? `${data.user.assignedGradeName} — Sección ${data.user.assignedSectionName}`
      : "Aula asignada por administración";

  return (
    <div className="space-y-8 text-foreground">
      <section className="flex flex-wrap items-end justify-between gap-4 rounded-2xl border border-secondary/10 bg-white p-6 shadow-sm">
        <div className="flex min-w-0 flex-1 flex-wrap items-start gap-4">
          <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-full border-2 border-accent/60 bg-cream">
            <Image src={institutionDefaults.logoPath} alt="" fill className="object-cover" sizes="64px" />
          </div>
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-secondary">Sistema Integral · Aula Virtual Docente</p>
            <h3 className="text-2xl font-bold tracking-tight text-primary">Panel Académico Docente</h3>
            <p className="mt-1 text-sm text-foreground/65">
              Bienvenido(a), <span className="font-semibold text-primary">{displayName}</span>. {aula}
            </p>
            <p className="text-xs text-secondary">{institutionDefaults.fullLegalName}</p>
          </div>
        </div>
        <div className="flex flex-wrap gap-3">
          <button
            type="button"
            className="flex cursor-pointer items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-white shadow-md transition hover:brightness-110"
          ...
          <button
            type="button"
            className="flex cursor-pointer items-center gap-2 rounded-xl border-2 border-primary bg-white px-5 py-2.5 text-sm font-semibold text-primary shadow-sm transition hover:bg-cream/50"
          ...
          <button
            type="button"
            className="flex cursor-pointer items-center gap-2 rounded-xl bg-accent px-5 py-2.5 text-sm font-semibold text-primary shadow-md ring-1 ring-primary/10 transition hover:brightness-95"
            onClick={() => router.push("/ia-sesiones")}
          >
            <Sparkles size={18} /> Sesión con IA
          </button>
        </div>
      </section>

      <div className="flex flex-wrap items-center justify-end gap-2 border-b border-secondary/10 pb-4">
        <Link href="/reportes" className="rounded-full p-2 text-secondary transition hover:bg-cream/60" aria-label="Reportes">
          <Bell size={20} />
        </Link>
        <Link href="/ia-sesiones" className="rounded-full p-2 text-secondary transition hover:bg-cream/60" aria-label="Ayuda e IA">
          <CircleHelp size={20} />
        </Link>
      </div>
      ...
      <section className="grid grid-cols-1 gap-6 md:grid-cols-4">
        {(
          ...
          <article
            key={title}
            className={`rounded-2xl border border-secondary/10 border-l-4 ${border} bg-white p-6 shadow-md transition hover:shadow-lg`}
          >
      ...
      <section className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="flex h-[400px] flex-col rounded-2xl border border-secondary/10 bg-white p-8 shadow-md lg:col-span-2">
            ...
            {[90, 95, 85, 92].map((h, i) => (
              <div key={i} className="flex w-full flex-col items-center gap-2">
                <div className="relative h-[80%] w-full rounded-t-lg bg-white/10">
                  <div className="absolute bottom-0 w-full rounded-t-lg bg-accent transition-all hover:brightness-110" style={{ height: `${h}%` }} />
                </div>
      ...
      <section className="overflow-hidden rounded-2xl border border-secondary/15 bg-white shadow-md">
